                ...
                if data_type == "test": # test set
                    ...
                    for rel in sample["relation_list"]:
                        ...
                        if subj_tok_span[0] >= start_ind and subj_tok_span[1] <= end_ind \
                            ...
                            new_rel["obj_tok_span"] = [obj_tok_span[0] - start_ind,
...
#             text = re.sub("\s+", " ", text).strip()
    ...
    def _get_ent2char_spans(self, text, entities, ignore_subword_match=True):
        '''
        if ignore_subword_match is true, find entities with whitespace around,
        e.g. "entity" -> " entity "
        '''
        ...
        for ent in entities:
            ...
            # m.span() 返回的是实体在原句中的首尾index, 这里确定实体的index会有一种情况，例如 实体绝世好B
            # 在电影《绝世好B》是是《绝世好BRA》的续集电影，实体会出现两个位置
            for m in re.finditer(re.escape(target_ent), text_cp):
                if not ignore_subword_match and re.match("\d+", target_ent): # avoid matching a inner number of a number
                    if (m.span()[0] - 1 >= 0 and re.match("\d",
                                                          text_cp[m.span()[0] - 1])) \
                            or (m.span()[1] < len(text_cp) and \
                                re.match("\d", text_cp[m.span()[1]])):
                        continue
                # 英文数据集上减2
                # span = [m.span()[0], m.span()[1] - 2] if ignore_subword_match else m.span()
                # 中文数据集上去掉减2，因为英文数据集上实体前后加空格，句子前后也加了空格
                span = [m.span()[0], m.span()[1]] if ignore_subword_match else m.span()
                spans.append(span)
#             if len(spans) == 0:
#                 set_trace()
    ...
    def add_char_span(self, dataset, ignore_subword_match=True):
        ...
        for sample in tqdm(dataset, desc="adding char level spans"):
            ...
            # 中文百度数据集中包含entity_list，这里的entities添加要做判断
            if "entity_list" in sample:
                entities.extend([ent["text"] for ent in sample["entity_list"] if ent["text"] not in entities])
            ent2char_spans = self._get_ent2char_spans(sample["text"],
                                                      entities,
            ...
            if len(sample["relation_list"]) > len(new_relation_list):
                ...
                miss_sample_list_save.append({"text": sample["text"],
                                              "relation_list": sample["relation_list"],
                                              "new_relation_list": new_relation_list})
        ...
        return dataset, miss_sample_list, miss_sample_list_save
    ...
    def add_tok_span(self, dataset):
        ...
        for sample in tqdm(dataset, desc="adding token level spans"):
            ...
            # print("char2tok_span: ", char2tok_span)

            for rel in sample["relation_list"]:
                subj_char_span = rel["subj_char_span"]
                # print("subj_char_span: ", subj_char_span)
                obj_char_span = rel["obj_char_span"]
                # print("obj_char_span: ", obj_char_span)
            ...
            if "event_list" in sample:
                for event in sample["event_list"]:
                    event["trigger_tok_span"] = char_span2tok_span(event["trigger_char_span"],
                                                                   char2tok_span)
